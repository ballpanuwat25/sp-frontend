...
function EditEquipmentCategory({ logout }) {
    ...
    const getEquipmentCategoryById = async () => {
        const response = await axios.get(process.env.REACT_APP_API + `/equipmentCategory-list/${id}`);
    ...
    const updateEquipmentCategory = async (e) => {
        e.preventDefault();
        const response = await axios.patch(process.env.REACT_APP_API + `/equipmentCategory-list/${id}`, {
        ...
        if (response.data.Error) {
            ...
            navigate("/chem/equipmentCategory-list");
        }
    }
